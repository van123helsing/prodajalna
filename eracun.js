  ...
  var form = new formidable.IncomingForm();
    ...
    pesmiIzRacuna(polja.seznamRacunov, function(pesmi){
      strankaIzRacuna(polja.seznamRacunov, function(stranka){
        ...
        odgovor.render('eslog', {vizualiziraj: true, postavkeRacuna: pesmi, str: stranka[0]});
      })
    })
  });
})
  ...
  pesmiIzKosarice(zahteva, function(pesmi) {
    if (!pesmi) {
      ...
      odgovor.send("<p>V košarici nimate nobene pesmi, \
        ...
        vrniStranko(zahteva.session.stevilkaPrijavljenegaGosta, function(napaka, stranke){
        if(napaka){
          odgovor.end();  
        } else{
            odgovor.setHeader('content-type', 'text/xml');
...
streznik.post('/prijava', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    ...
    if(napaka1 || napaka2){
      response = "Prišlo je do napake pri registraciji nove stranke. Prosim preverite vnešene podatke in poskusite znova.";
      odgovor.redirect("/prijava");
    }
    else{
      response = "Stranka je bila uspešno registrirana.";
      odgovor.redirect("/prijava");
    }
...
streznik.get('/prijava', function(zahteva, odgovor) {
  vrniStranke(function(napaka1, stranke) {
      vrniRacune(function(napaka2, racuni) {
        odgovor.render('prijava', {sporocilo: response, seznamStrank: stranke, seznamRacunov: racuni});  
      }) 
    });
})
...
streznik.post('/stranka', function(zahteva, odgovor) {
  var form = new formidable.IncomingForm();
  zahteva.session.isLogedIn = true;
  form.parse(zahteva, function (napaka1, polja, datoteke) {
...
streznik.post('/odjava', function(zahteva, odgovor) {
    zahteva.session.isLogedIn = false;
    odgovor.redirect('/prijava') 
})
